...
class Encoder_M(nn.Module):
    def __init__(self, arch):
        ...
        # 从预训练模型复制权重，并添加一个随机初始化的通道
        with torch.no_grad():
            self.conv1.weight[:, :3, :, :] = resnet.conv1.weight.clone()
            # 使用正态分布初始化第4个通道
            nn.init.normal_(self.conv1.weight[:, 3:, :, :], mean=0.0, std=0.01)
        self.bn1 = resnet.bn1
        ...
        self.register_buffer('mean', torch.FloatTensor([0.485, 0.456, 0.406, 0.0]).view(1,4,1,1))
        self.register_buffer('std', torch.FloatTensor([0.229, 0.224, 0.225, 1.0]).view(1,4,1,1))
...
class Encoder_Q(nn.Module):
    def __init__(self):
        ...
        with torch.no_grad():
            ...
            std = math.sqrt(2. / n)
            self.conv1.weight[:, 3].normal_(0, std)
    ...
    def forward(self, in_f):
        ...
        f8 = self.res3(f4) # 1/8, 512
        f16 = self.res4(f8) # 1/16, 1024
    ...
    def load_param(self, weight):
        ...
        for key, val in weight.items():
            if key in s and s[key].shape == val.shape:
                ...
                print('ignore weight of mistatched shape in key {}'.format(key))
