...
class Davis16(BaseData):
    ...
    def __init__(self, train=True, sampled_frames=3, 
        ...
        self.feature_dir = os.path.join(data_dir, 'feature_map_dir', 'features')  # 新增特征目录
    ...
    def __getitem__(self, idx):
        ...
        feature_folder = os.path.join(self.feature_dir, vid)  # 新增特征路径
        ...
        for name in sample_frame:
            feature_file = os.path.join(feature_folder, f"{int(name):06d}.npy")  # 新增特征文件
            if os.path.exists(feature_file):
                feature = np.load(feature_file)
                features.append(feature)
            else:
                print(f"Warning: Feature file not found: {feature_file}")
                features.append(np.zeros((frame[0].shape[0], frame[0].shape[1], 4), dtype=np.float32))
        ...
        try:
            frame, mask, features = self.transform(frame, mask, features)
        ...
        return frame, mask, features, num_obj, info
...
class DaliDavis17(Davis17):
    ...
    def __getitem__(self, idx):
        ...
        return frame, mask, [np.array([num_obj])] * nsamples, [np.array([self.max_obj])] * nsamples, features
